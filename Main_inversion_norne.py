...
import pandas as pd
from GAN_module import Generator_norne, DiscriminatorWNet_norne
...
def main(args):
    ...
    def CLoss(image):
        ...
        masked= masked*image
        loss = torch.linalg.norm((masked - conditioning_data_G).reshape(-1, args.nz * args.nx), ord=2, dim=1, keepdim=True).mean()
        return loss
    ...
    #Define GAN's G and D

    G = Generator_norne().to(device)
    D = DiscriminatorWNet_norne().to(device)
    ...
    if seismic.ndim!=2: seismic= np.flip(seismic[:,y_idx,:].T, axis=0).copy().T
    
    args.max_abs_amp= np.abs(seismic).max()
  
    FM.real_seismic[:,:]= torch.tensor(seismic.T/args.max_abs_amp).to(device)
    
    if (args.precompute_TI_seismic==False):
        if os.path.isdir(args.project_path+args.TI_path+'/Seismic_TI'): 
            shutil.rmtree(args.project_path+args.TI_path+'/Seismic_TI')
        if os.path.isdir(args.project_path+args.TI_path+'/Facies_TI'): 
            shutil.rmtree(args.project_path+args.TI_path+'/Facies_TI')
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, data in enumerate(dataloader):
            ...
            #transform fake image to avoid divergence
            ...
            _, _, score_S = D(fake_image, realseis)

            loss_D_FS= loss(score_FS_TI, real) + loss(score_FS_fake, fake)
            loss_D_S= loss(score_S_TI, real) + loss(score_S_fake,real)+ loss(score_S,real)
            ...
            loss_F = loss(score_F, real)#*(1-score_FS.mean().item())
...
if __name__=='__main__':
  ...
  for cc in [1]:
    ...
    parser.add_argument("--in_folder", default='input_norne/',
                        type=str, help="input folder"
                        )
    parser.add_argument("--TI_path", default='dataset_norne/Facies_2/',
                        type=str, help="Training Images folder"
                        )
    parser.add_argument("--well_data", default='well_data',
                        type=str, help="Well data filename"
                        )
    parser.add_argument("--real_seismic_file", default='/Seismic3D',
                        type=str, help="seismic file name"
                        )
    parser.add_argument("--wavelet_file", default='wavelet_rick',
                        type=str, help="Name of the Wavelet file"
                        )
    parser.add_argument("--W_weight", default=13000,
    ...
    parser.add_argument("--lr_steps",default=50,
                        type=float, help="learning rate decay"
                                    )
    parser.add_argument("--alpha_c", default=cc, 
    ...
    parser.add_argument("--precomputed", default=True,
    ...
    parser.add_argument("--nx", default=109,
                        type=int, help='x size of inversion grid'
                        )
    parser.add_argument("--ny", default=79,
                        type=int, help='y size of inversion grid'
                        )
    parser.add_argument("--nz", default=75,
    ...
    parser.add_argument("--var_range", default=[[[13,13]],[[13,13]]],
    ...
    if not os.path.isdir(parser.parse_args().project_path+out_folder):
            ...
            os.mkdir(parser.parse_args().project_path+out_folder+'/eval_train/facies')
